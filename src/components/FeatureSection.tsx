...
const FeatureSection = () => {
   ...
   return (
      <section ref={ref} id="features" className="relative py-20 bg-[#ffffff]">
         <div className="absolute inset-0 overflow-hidden">
            <motion.div
               initial={{ opacity: 0, scale: 0.5 }}
               animate={isInView ? { opacity: 0.2, scale: 1 } : {}}
               transition={{ duration: 1.5 }}
               className="absolute right-0 top-1/4 w-96 h-96 bg-purple-600/40 rounded-full blur-[100px]"
            />
            <motion.div
               initial={{ opacity: 0, scale: 0.5 }}
               animate={isInView ? { opacity: 0.2, scale: 1 } : {}}
               transition={{ duration: 1.5, delay: 0.2 }}
               className="absolute left-0 bottom-1/4 w-80 h-80 bg-blue-900/40 rounded-full blur-[100px]"
            />
         </div>
