...
import { 
  ...
  Clock, TrendingUp, CreditCard, Share2, Twitter, Download, Copy
} from "lucide-react";
...
type OfframpStatus = "PENDING" | "PROCESSING" | "COMPLETED" | "FAILED" | "CANCELLED";
...
const TokenToNGNConfirmModal: React.FC<TokenToNGNConfirmModalProps> = ({
  ...
  const [offrampProgress, setOfframpProgress] = useState<OfframpProgressData | null>(null);
  ...
  const startOfframpPolling = async (orderId: string): Promise<boolean> => {
    try {
      const response = await fetch(`${API_BASE_URL}/ramp/offramp/${orderId}/start-polling`, {
        method: 'POST',
        headers: {
      ...
      logger.log('ERROR', `Failed to start polling for order ${orderId}`, error);
      return false;
    }
  };
  ...
  const stopOfframpPolling = async (orderId: string): Promise<boolean> => {
    try {
      const response = await fetch(`${API_BASE_URL}/ramp/offramp/${orderId}/stop-polling`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
      ...
      logger.log('ERROR', `Failed to stop polling for order ${orderId}`, error);
      return false;
    }
  };
  
  const checkOfframpStatus = async (orderId: string): Promise<OfframpProgressData | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/ramp/offramp/status/${orderId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
      ...
      logger.log('STATUS_CHECK', `Checked status for order ${orderId}`, progressData);
      return progressData;
    } catch (error) {
      logger.log('ERROR', `Failed to check status for order ${orderId}`, error);
      return null;
    }
  };
  
  // Polling logic for offramp progress
  const startProgressPolling = useCallback(async (orderId: string) => {
    if (isPollingActive) return;
    
    setIsPollingActive(true);
    logger.log('POLLING', `Starting progress polling for order ${orderId}`);
    ...
    const interval = setInterval(async () => {
      ...
      if (progressData) {
        ...
        if (progressData.status === 'COMPLETED') {
          logger.log('CONVERSION', `NGN conversion completed for order ${orderId}`);
          ...
          logger.log('ERROR', `NGN conversion failed for order ${orderId}`, progressData);
          setError(`Conversion failed: ${progressData.currentStep || 'Unknown error'}`);
  ...
  const generateReceiptImage = async (): Promise<string | null> => {
    ...
    try {
      ...
      receiptElement.style.background = 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 50%, #3730a3 100%)';
          ...
          <div>
            ...
            <div style="background: rgba(255,255,255,0.2); border-radius: 20px; padding: 30px; margin-bottom: 40px;">
              ...
              <div style="font-size: 24px; opacity: 0.9;">
                Swapped from ${formatCurrency(swapDetails.fromAmount, swapDetails.fromToken)}
              </div>
              <div style="margin-top: 30px; text-align: left;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                  <span style="opacity: 0.9;">Rate:</span>
                  <span style="font-weight: 600;">1 ${swapDetails.fromToken} = ${swapDetails.rate} ${swapDetails.toToken}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
        ...
        {needsApproval && (
          <div className="flex items-center">
            <div className={`flex items-center justify-center w-7 h-7 rounded-full ${
              swapStep === "approval" 
                ? "bg-blue-500" 
                : (swapStep === "swap" || swapStep === "converting" || swapStep === "complete") 
                  ? "bg-green-500" 
                  : "bg-gray-200 dark:bg-gray-700"
            } text-white text-sm mr-2`}>
              {swapStep === "approval" && transactionStatus === "pending" ? (
                <Loader className="animate-spin" size={14} />
              ) : (swapStep === "approval" || swapStep === "swap" || swapStep === "converting" || swapStep === "complete") ? (
                <CheckCircle size={14} />
              ) : (
                <span>2</span>
              )}
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-blue-400">Converting to NGN</h3>
            <p className="text-sm text-blue-300">
              {offrampProgress.currentStep || "Processing your conversion..."}
            </p>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-blue-900/30 rounded-full h-2">
            <div 
              ...
              style={{ width: `${offrampProgress.progress}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-blue-400 font-medium">{offrampProgress.status}</p>
            </div>
          </div>
          {offrampProgress.estimatedTime && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-blue-400 font-medium">{offrampProgress.estimatedTime}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {offrampOrderId && (
          <div className="mt-4 pt-4 border-t border-blue-900/30">
            <p className="text-xs text-blue-400">
              Order ID: <span className="font-mono">{truncateAddress(offrampOrderId)}</span>
            </p>
          </div>
        )}
      </motion.div>
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-blue-400">Share Your Success!</h3>
            <p className="text-sm text-blue-300">
              {swapStep === "complete" ? "Let the world know about your successful swap!" : "Share your ongoing transaction with friends!"}
            </p>
          </div>
        </div>
          ...
          <motion.div
            ...
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-bold">
                {swapStep === "complete" ? "Swap Successful!" : 
                 swapStep === "converting" ? "Converting to NGN" : 
                 "Confirm Token Swap"}
              </h2>
              <button
                onClick={onClose}
                disabled={transactionStatus === "pending" || loadingDepositAddress || isPollingActive}
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    {!isEditingAccount && (
                      <button 
                        ...
                        disabled={transactionStatus === "pending" || isPollingActive}
            ...
            {swapStep === "complete" && (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-green-400">Conversion Complete!</h3>
                ...
                {offrampProgress && (
                  <div className="grid grid-cols-2 gap-4 text-sm pt-3 border-t border-green-900/30">
                    <div>
                      ...
                      <p className="text-green-400 font-medium">₦{swapDetails.toAmount}</p>
                    </div>
                    <div>
                      <p className="text-green-300">Transaction Time</p>
                      <p className="text-green-400 font-medium">
                        {offrampProgress.lastUpdated ? 
                          new Date(offrampProgress.lastUpdated).toLocaleTimeString() : 
                          'Just now'}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-400 mt-2 font-medium">
                      The NGN conversion will happen automatically after the swap is complete.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {swapStep === "converting" && (
              <div className="bg-indigo-900/20 rounded-lg p-4 mb-5 border border-indigo-900/50">
                <div className="flex items-start gap-2">
                  <CreditCard className="text-indigo-400 mt-0.5" size={18} />
                  <div>
                    <p className="text-sm text-indigo-400">
                      Your tokens have been successfully swapped! We're now converting to NGN and sending to your bank account.
                    </p>
                    <p className="text-sm text-indigo-400 mt-2 font-medium">
                      This process typically takes 2-5 minutes. You'll be notified once complete.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {(!bankDetails?.accountNumber || bankDetails.accountNumber.length < 10) && !isEditingAccount && swapStep !== "complete" && (
            ...
            <div className="flex gap-3 mt-6 sticky bottom-0 pb-2 pt-2 bg-gray-900">
              <button
                ...
                {swapStep === "complete" ? "Close" : 
                 swapStep === "converting" ? "Minimize" : 
                 "Cancel"}
              </button>
